https://www.lualearning.org/tutorials/043d2777-1297-43bf-bc8b-434418e92658

Most tutorials on YouTube are outdated (using `LegacyChatService`), so I decided to make one using updated methods (using `TextChatService`).
...
Because we have PlayerAdded/Removing detection script, we can use RemoteEvents to send data (Player.Name has joined/left.) from Server (the script we just added in `ServerScriptService`) to Client (where the chat message will be displayed).
- Add a Folder in `ReplicatedStorage` and name it as *PlayerEvents* (or whatever you like).
- Add 2 RemoteEvent named *Joining* and *Leaving* (or again whatever you like) 
...
```
>**5. More:**
And that's it.. Play on Client (not Studio) and it should work as intended.
...
----
Feel free to leave your opinions and suggestions. If you have any questions, feel free to ask!
Thanks for reading my first tutorial  ☺️  (greetings from 🇻🇳).
